...
function setupSocketIO(ctx: AdminContext): void {
    ...
    ctx.io.use(async (socket: any, next: (err?: Error) => void) => {
        ...
        // 握手时同样通过平台校验 token（带短缓存，与 HTTP 中间件共享）
        try {
            ...
            socket.data.platformCaller = { account: result.account, systemName: result.systemName };
            return next();
        } catch {
            // 平台不可达时降级拒绝连接，避免未认证流量穿透
            return next(new Error('认证服务暂不可用，请稍后重试'));
        }
    });
